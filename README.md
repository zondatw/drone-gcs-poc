...
### 每台不同航線同時跑 + 亂數航線
側欄「**群組任務**」卡讓每台各飛各的:
- **🎲 全部亂數航線**:幫每台各自生一條隨機航線(繞自己起飛點 80–250m);單台版在「航線規劃」卡也有「🎲 亂數航線」。
- **▶ 全部開始任務**:把每台**各自**的航線上傳並開始(前端 `Promise.all` 對每台 `missionUpload(i)→missionStart(i)`),
  N 台同時各跑各的路線。**‖ 全部暫停**一鍵全停。
- 地圖會用**每台的顏色**畫出各自航線(控制中那台是黃色、可拖曳/右鍵編輯)。
